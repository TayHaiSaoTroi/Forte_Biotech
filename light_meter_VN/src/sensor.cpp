...
void sensor::handle_Sensor()
{
    ...
    for (int i = 0; i < numSensor; i++)
    {
        ...
        else if (value_sensor >= 9000)
            this->result_Sensor[i][_displayCLD.couter - 1] = 9000;
        else if(value_sensor >valueMAXsensor && value_sensor <9000)
            this->result_Sensor[i][_displayCLD.couter -1]=value_sensor;
        else
