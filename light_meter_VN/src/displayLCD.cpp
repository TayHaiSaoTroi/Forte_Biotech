...
switch (step) {
  ...
  this->display->print("Nhấn nút đỏ để chọn ống cần đo");
  if(butt == 0){
    this->display->setTextColor(Forte_Green);
  }
  else{
  ...
  this->display->setCursor(5, 230);
  this->display->print("Nhấn nút xanh để tiếp tục");
}
...
void displayCLD::prepare() {
  ...
  this->display->setTextColor(Forte_Green);
  this->display->setCursor(5, 60);
  this->display->println("Đặt ống mẫu vào máy và đậy nắp");
  // this->display->drawRect(0, 0, 320, 240, Forte_Green);
  // this->display->drawRect(185, 0, 135, 35, Forte_Green);
  this->display->drawRect(30, 140, 252, 80, RED);
  this->display->drawRect(29, 139, 254, 82, RED);
...
void displayCLD::screen_Average_Result() {
  ...
  this->display->setTextColor(RED);
  this->display->print("Nút đỏ:Gửi kq đo");
}
